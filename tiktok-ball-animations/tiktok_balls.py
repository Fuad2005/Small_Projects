"""
Every time ball gets out of the circle, 2 new are spawned
"""
...
def is_ball_in_arc(ball_pos, CICRCLE_CENTER, start_angle, end_angle):
    ...
    for ball in balls:
        ...
        if dist + BALL_RADIUS > CIRCLE_RADIUS:
            ...
            if ball.is_in:
                ...
                proj_v_t = (np.dot(ball.vel, tangent) / np.dot(tangent, tangent)) * tangent
                ball.vel = 2*proj_v_t - ball.vel

                ball.vel *= 0.95 # friction
                ball.vel += tangent * spinning_speed
